...
def create_signed_vc(tourist_data, issuer_private_key, issuer_did, verification_method):
    ...
    payload_bytes = json.dumps(credential_payload, separators=(',', ':')).encode('utf-8')
...
# --- THE FIX: Create a single entry point for the API ---
async def issue_tourist_credential(tourist_data):
    ...
    """
    try:
        # 1. Generate the issuer's identity
        private_key, issuer_did, verification_method = generate_issuer_id()

        # 2. Use the identity to create and sign the credential
